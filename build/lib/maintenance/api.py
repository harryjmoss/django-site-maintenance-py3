...
MAINTENANCE_FILE = getattr(settings, 'MAINTENANCE_FILE', f'/tmp/DJANGO_MAINTENANCE_FILE_{settings.SITE_ID}')
    ...
    if not ignore_session:
        try:
            ...
            while True:
                ...
                if not rounds:
                    if verbosity > 0:
                        print(f"Active sessions detected. Waiting for logout. (Session timeout set to {settings.SESSION_COOKIE_AGE} secs)")
...
def check():
    return 0, f"Status: {STATUS._labels[status()]} - Active sessions: {get_active_users()}"
