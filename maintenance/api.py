...
MAINTENANCE_FILE = getattr(
    settings, "MAINTENANCE_FILE", f"/tmp/DJANGO_MAINTENANCE_FILE_{settings.SITE_ID}"
)
PENDING_MAINTENANCE_FILE = f"{MAINTENANCE_FILE}_"
MAINTENANCE_URL = getattr(
    settings, "MAINTENANCE_URL", static("maintenance/maintenance.html")
)
        ...
        try:
            ...
            while True:
                ...
                if not rounds:
                    if verbosity > 0:
                        print(
                            f"Active sessions detected. Waiting for logout. (Session timeout set to {settings.SESSION_COOKIE_AGE} secs)"
                        )
                        print("Type double ^C to stop")
                        print(
                            f"{users} pending sessions. {C[rounds % 2]} ({round(time.time() - _start)} sec)\r"
                        )
                rounds += 1
                if rounds >= timeout:
...
def check():
    return (
        0,
        f"Status: {STATUS._labels[status()]} - Active sessions: {get_active_users()}",
    )
...
def is_pending():
    ...
    return os.path.exists(PENDING_MAINTENANCE_FILE) and not os.path.exists(
        MAINTENANCE_FILE
    )
