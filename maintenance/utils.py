...
from maintenance.management.lockfile import (
    AlreadyLocked,
    FileLock,
    LockFailed,
    LockTimeout,
...
def fmt_duration(secs):
    pluralize = lambda value, label: "{}{}".format(label, ["", "s"][value > 1])
...
class CommandTask(object):
    ...
    def __init__(self, session_key, force=False, timeout=1, check_maintenance=False):
        self._key = "{}.lock".format(session_key)
